...
fn find_module_by_name(graph: &DependencyGraph, module_name: &str) -> Result<ModuleIdentifier> {
    ...
    Err(anyhow::anyhow!("Module '{}' not found in dependency graph", module_name))
}
...
fn analyze_dependency_modules(
    ...
    // Filter out test modules
    dependency_module_names.retain(|module_path| {
        !module_path.contains(".tests.") && !module_path.ends_with(".tests")
    });
...
fn get_import_dependents(graph: &DependencyGraph, target_module: &ModuleIdentifier) -> Result<Vec<String>> {
    ...
    for module in graph.all_modules() {
        ...
        for (dep_module, dep_type) in dependencies {
            ...
            if dep_type == crate::graph::DependencyType::Imports && dep_module == target_module.canonical_path {
...
fn get_import_dependencies(graph: &DependencyGraph, target_module: &ModuleIdentifier) -> Result<Vec<String>> {
...
fn generate_test_order(
    ...
    deps.sort_by(|a, b| b.risk_level.cmp(&a.risk_level).then(a.module_name.cmp(&b.module_name)));
    ...
    affected.sort_by(|a, b| {
        ...
        a.dependency_depth.cmp(&b.dependency_depth)
...
pub mod formatters {
    ...
    fn format_modules_by_risk(modules: &[super::ChangesetModule]) -> String {
        ...
        for module in modules {
            by_risk.entry(module.risk_level.clone()).or_default().push(module);
        }
        ...
        for risk_level in [RiskLevel::Critical, RiskLevel::High, RiskLevel::Medium, RiskLevel::Low] {
            if let Some(risk_modules) = by_risk.get(&risk_level) {
                ...
                output.push_str(&format!("{} {:?} Risk ({} modules):\n", risk_icon, risk_level, risk_modules.len()));
